...
let finalizeCalls=0;
window.fixtureCalls=[];
window.__TAURI_INTERNALS__={metadata:{currentWindow:{label:'main'},currentWebview:{label:'main'}},transformCallback:(fn)=>{let id=++callbackId;callbacks.set(id,fn);return id;},unregisterCallback:(id)=>callbacks.delete(id),convertFileSrc:()=>fixtureAvatar,invoke:async(cmd,args={})=>{
 ...
 case 'auth_generate_code':return {code:'ABCDE',generatedAt:Math.floor(Date.now()/1000),periodRemaining:30-(Math.floor(Date.now()/1000)%30)};
 ...
 case 'auth_confirmation_details':if(scenario==='details-error')throw 'CONF_DETAILS_UNAVAILABLE';return {partnerSteamId:'76561198000000002',giving:[{assetId:'11',appId:730,name:'Dreams & Nightmares Case',amount:'2',icon:itemIcon}],receiving:[{assetId:'12',appId:730,name:'Demo item with a long name to check wrapping',amount:'1',icon:brokenIcon}]};
 ...
 case 'list_running_sandboxes':return scenario==='layout'?[{login:'demo_sandbox',boxName:'SteamShadow_demo',pids:[1],startedAt:Math.floor(Date.now()/1000)-1080}]:[];
